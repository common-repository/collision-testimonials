...
function testimonials_install(){
	...
	if ($wpdb->get_var("show tables like '$testimonials'") != $testimonials){ // No, it doesn't
		...
		$quote = "Congratulations, you have completed the installation!";
		$website = "http://www.backendlabs.com/";
		
		// Inserting testimonial into the DB
		$insert = "INSERT INTO " . $testimonials .
		" (id, status, featured, name, location, quote, website) " .
		"VALUES ('','0','0','" . $wpdb->escape($date) . "','" . $wpdb->escape($name) . "','" . $wpdb->escape($location) . "','" . $wpdb->escape($quote) . "','" . $wpdb->escape($website) . "')";
...
function collision_testimonials($id=""){
	...
	if (!empty($id)){ // If the id has been set, show that testimonial instead!
		...
		while ($data = mysql_fetch_array($results)){
			...
			// Final HTML Output
			$theTestimonials .= $testimonial_prefix . $quote . '<br /><span>&mdash; <strong>' . $websitepre . $name . $websitesuf . '</strong><em>' . $location . "</em></span>" . $testimonial_suffix;
		}
		...
		while ($data = mysql_fetch_array($results)){
			...
			// Final HTML Output
			$theTestimonial = $testimonial_prefix . $quote . '<br /><span>&mdash; <strong>' . $websitepre . $name . $websitesuf . '</strong><em>' . $location . "</em></span>" . $testimonial_suffix;
...
function collision_testimonials_form(){
	...
	if (isset($_POST['submit'])){ // check if form has been submitted
		...
		if (empty($clientname) || empty($email) || empty($testimonial) || empty($captcha_value)){
		...
		if (!count($formerrors)){
			...
			$insert = "INSERT INTO " . $testimonials .
			" (id, status, name, location, quote, website) " .
			"VALUES ('','" . $wpdb->escape('2') . "','" . $wpdb->escape($clientname) . "','" . $wpdb->escape($location) . "','" . $wpdb->escape($testimonial) . "','" . $wpdb->escape($website_final) . "')";
			$results = $wpdb->query($insert);
