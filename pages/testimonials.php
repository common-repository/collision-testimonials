...
<?php
	...
	if (isset($_POST['addQuote'])){
		...
		if (!empty($name) && !empty($quote)){
			...
			" (id, status, date, name, location, quote, website) " .
			"VALUES ('', '" . $wpdb->escape($status) . "', '" . $wpdb->escape($currdate) . "', '" . $wpdb->escape($name) . "', '" . $wpdb->escape($location) . "', '" . $wpdb->escape($quote) . "', '" . $wpdb->escape($website_final) . "')";
	...
	if (isset($_POST['editQuote'])) {
		...
		if (!empty($name) && !empty($quote)){
			...
			" SET status='" . $wpdb->escape($status) . "', name='" . $wpdb->escape($name) . "', location='" . $wpdb->escape($location) . "', quote='" . $wpdb->escape($quote) . "', website='" . $wpdb->escape($website_final) . "' WHERE id='". $id ."'";
...
<form method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">	
	...
	<thead>
		...
		<?php if ($_GET['dir'] == "desc" || !isset($_GET['dir']) || empty($_GET['dir'])){ ?>
			...
			<th class="column_id" id="id" scope="col" style="width:30px;"><a href="admin.php?page=testimonials&sort=featured&dir=asc"><img src="<?php $image = get_option('siteurl') . '/wp-content/plugins/collision-testimonials/i/images/star.png'; echo $image; ?>" alt="" style="width:18px;" /></a></th>
		...
		<?php } else { ?>
			...
			<th class="column_id" id="id" scope="col" style="width:30px;"><a href="admin.php?page=testimonials&sort=featured&dir=desc"><img src="<?php $image = get_option('siteurl') . '/wp-content/plugins/collision-testimonials/i/images/star.png'; echo $image; ?>" alt="" style="width:18px;" /></a></th>
			...
			<th class="column-website" id="website" scope="col" style="width:200px;"><a href="admin.php?page=testimonials&sort=website&dir=desc">Website</a></th>
...
<?php } ?>
	...
	<tfoot>
		<tr class="thead">
			...
			<th class="column_id" id="id" scope="col"><img src="<?php $image = get_option('siteurl') . '/wp-content/plugins/collision-testimonials/i/images/star.png'; echo $image; ?>" alt="" style="width:18px;" /></th>
